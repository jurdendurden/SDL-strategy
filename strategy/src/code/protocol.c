...
void init_mxp(DESCRIPTOR_DATA *d)
{
  ...
  write_to_buffer( d, MXPTAG 
   ...
   //Obj info tags:
   //WTB ( d, MXPTAG
   //	("!ELEMENT ObjInfo \"<send" "hint='Name &name;'>\" "
      
   WTB ( d, MXPTAG		
      ("!ELEMENT Help \"<send href='help &#39;&name;&#39; ' "
       "hint='Read help file for: &name;'>\" "
       "ATT='name'"), 	0 );

   
   //OLC tags
   //WTB (d, MXPTAG
   //("!ELEMENT OLCFlag \"<send href='
}
...
void do_mxp(ACCOUNT_DATA * account, char *argument)
{
  if (!account->d)
    return;
  ...
  if (account->d->mxp)
  {
    shutdown_mxp(account->d);
    SEND("MXP has been disabled.\n\r", account);
    return;
  }
  else
  {   
      init_mxp(account->d);
      SEND("MXP has been enabled.\n\r", account);
   }
}
 ...
 *
 * These are per-account so that they can be customised for different characters 
 * (eg changing 'mana' to 'blood' for vampires).  You could even allow accounts 
...
void ProtocolInput( descriptor_t *apDescriptor, char *apData, int aSize, char *apOut )
{
   ...
   /* Copy the input buffer back to the account. */
   strcat( apOut, CmdBuf );
}
...
static void Negotiate( descriptor_t *apDescriptor )
{
   ...
   {
      const char RequestTTYPE   [] = { (char)IAC, (char)SB,   TELOPT_TTYPE, SEND_IT, (char)IAC, (char)SE, '\0' };
...
static void PerformHandshake( descriptor_t *apDescriptor, char aCmd, char aProtocol )
{
   ...
   if (bResult == FALSE)
   {
      log_string("Perform Handshake, bResult = FALSE");
   }
}
      ...
      case (char)TELOPT_TTYPE:
         if ( pProtocol->bTTYPE )
         {
            ...
            if ( !strcmp(pProtocol->pVariables[eMSDP_CLIENT_ID]->pValueString, "Unknown") )
            {
                ...
                * the account losing echo, but it has other issues.  Because the 
                ...
                * until the account has entered their name before negotiating.
                */
            ...
            {
               char RequestTTYPE [] = { (char)IAC, (char)SB, TELOPT_TTYPE, SEND_IT, (char)IAC, (char)SE, '\0' };
...
static void ExecuteMSDPPair( descriptor_t *apDescriptor, const char *apVariable, const char *apValue )
{
   ...
   {
      if ( MatchString(apVariable, "SEND_IT") )
      {
         bool bDone = false;
         int i; /* Loop counter */
      ...
      else if ( MatchString(apVariable, "LIST") )
      {
         ...
         {
            const char MSDPCommands[] = "LIST REPORT RESET SEND_IT UNREPORT";
            ...
            if ( VariableNameTable[i].bConfigurable )
            {
               if ( MatchString(apVariable, VariableNameTable[i].pName) )
               {
                  if ( VariableNameTable[i].bString )
                  {
                      ...
                      * where we don't really want the account overwriting a 
...
static void SendMSSP( descriptor_t *apDescriptor )
{
   ...
   static MSSP_t MSSPTable[] =
   {
      ...
      { "HOSTNAME",           "mudhaven.net" },
      { "PORT",               "9000" },
      { "CODEBASE",           "ROM 2.4b6 (Heavily modified, over 150k lines of code)" },
